...
export default function BookingCart({
    ...
    const createBookingMutation = useMutation({
        ...
        onSuccess: async (response) => {
            ...
            toast({
                title: t("booking_cart.booking_confirmed"),
        ...
        onError: (error) => {
            ...
            toast({
                title: t("booking_cart.booking_failed"),
    ...
    const handleServiceDurationChange = (
        ...
        const updatedServices = selectedServices.map((service) =>
            service.id === serviceId
                ? { ...service, duration: Math.max(1, duration) }
                : service
        );
    ...
    const handleBooking = () => {
        if (!isAuthenticated) {
            toast({
                title: t("booking_cart.login_required"),
        ...
        if (!bookingData.checkIn || !bookingData.checkOut) {
            toast({
                title: t("booking_cart.missing_information"),
        ...
        if (bookingData.guests > property.maxGuests) {
            toast({
                title: t("booking_cart.too_many_guests"),
        ...
        if (checkOutDate <= checkInDate) {
            toast({
                title: t("booking_cart.invalid_dates"),
        ...
        <Card className="p-6">
            <div className="mb-6">
                <h2
                    ...
                    data-testid="text-booking-cart-title"
                >
                    Book Your Stay
                </h2>
            ...
            {selectedServices.length > 0 && (
                <div className="mb-6">
                    <h3
                        ...
                        data-testid="text-selected-services-title"
                    >
                ...
                <div className="space-y-3 mb-6">
                    <div className="flex justify-between text-sm">
                        <span
                            ...
                            data-testid="text-property-nights"
                        >
                            $
                            {parseFloat(
                        ...
                        <span
                            ...
                            data-testid="text-property-subtotal"
                        >
                    ...
                    {servicesTotal > 0 && (
                        <div className="flex justify-between text-sm">
                            <span
                                className="text-muted-foreground"
                                data-testid="text-services-subtotal-label"
                            >
                                Services
                            </span>
                            <span
                                className="text-foreground"
                                data-testid="text-services-subtotal"
                            >
                    ...
                    {discountAmount > 0 && (
                        <div className="flex justify-between text-sm">
                            <span
                                className="text-muted-foreground"
                                data-testid="text-discount-label"
                            >
                                Bundle discount (
                                {selectedServices.length >= 3 ? "10%" : "5%"})
                            </span>
                            <span
            ...
            {/* Booking Button - Only show when authenticated */}
            {isAuthenticated ? (
                <Button
                ...
                >
                    ...
                    {createBookingMutation.isPending
                        ? t("booking_cart.processing")
                        : nights <= 0
                        ? t("booking_cart.select_dates")
                        : `Book for ${
                              totalAmount > 0
                                  ? `$${totalAmount.toLocaleString()}`
                                  : t("booking_cart.free")
